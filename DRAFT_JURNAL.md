...
Evaluasi dirancang dalam lima skenario yang masing-masing merepresentasikan tipe sumber dan dimensi transfer pengetahuan berbeda. Seluruh teks berbahasa Indonesia dan berasal dari data operasional nyata yang telah dianonimisasi dari platform BOND_SYS.
...
**Skenario B — PDF (L1, Explicit→Actionable):** Dokumen spesifikasi sistem BOND_SYS dalam format PDF/TXT. Lima pertanyaan mencakup alur proses bisnis yang terdokumentasi: tahapan Buyback Cash, perbedaan sesi General/Restricted, pihak dalam approval RFQ, persyaratan teknis Upload Allocation, dan mekanisme notifikasi broadcast.
...
**Skenario D — PDF+DB (L1+L2, Explicit→Cross-referenced):** Penggabungan dokumen PDF dan PostgreSQL 8 tabel melalui `MultiSourceAdapter`. Lima pertanyaan membutuhkan *cross-reference* dokumen spesifikasi dengan data aktual: konsistensi jam sesi, board type di dokumen vs DB, konsistensi offering_parameter, perhitungan settlement_date, dan konsistensi offering_digit dengan fraction_masters.

**Skenario E — Hybrid All (L1+L2+L3, Cross-Paradigm):** Penggabungan semua layer (PDF, PostgreSQL, log diskusi) dalam satu indeks FAISS. Lima pertanyaan membutuhkan ketiga layer sekaligus: bug submit quotation BS-SB (Chat+PDF), keputusan offering digit (Chat+DB), insiden ETL MOFIDS Februari 2023 (Chat), alur upload allocation (PDF+DB+Chat), dan status fitur amend (PDF+DB+Chat). Skenario ini satu-satunya yang dilengkapi **5 jawaban referensi (*ground truth*) yang disusun manual** oleh peneliti (`GROUND_TRUTH_HYBRID`), sehingga metrik ROUGE-L dan BLEU-1 bersifat *reference-based* — berbeda dari Skenario A–D yang *reference-free*.
...
Untuk membuktikan klaim "Multi-Sumber", evaluasi dirancang dalam lima skenario yang masing-masing merepresentasikan satu dimensi transfer pengetahuan organisasi. Skenario E secara khusus membuktikan klaim *source-agnostic* pada level paling tinggi — satu query menjangkau dokumen PDF/TXT spesifikasi sistem, log diskusi (TXT), **dan** tabel PostgreSQL (SQL) secara bersamaan dalam satu indeks FAISS. Skenario E juga satu-satunya skenario yang dilengkapi ground truth sehingga ROUGE-L dan BLEU-1 bersifat reference-based:
...
| B: PDF | L1 | `FolderSourceAdapter` | Spesifikasi sistem BOND_SYS (dokumen PDF/TXT) | TXT | Explicit → Actionable | Spesifikasi formal → insight proses bisnis |
| C: PostgreSQL | L2 | `PostgreSQLAdapter` | DB MOFIDS (8 tabel, 20 RFQ) | SQL | Explicit → Structured | Data tabel → narasi kontekstual |
| D: PDF+DB | L1+L2 | `MultiSourceAdapter` | PDF + PostgreSQL (gabungan) | TXT + SQL | Explicit → Cross-referenced | Verifikasi lintas dokumen spesifikasi dan data aktual |
| E: Hybrid All† | L1+L2+L3 | `MultiSourceAdapter` | PDF + DB + Chat (semua layer) | TXT + SQL | Cross-Paradigm | Sintesis tacit + explicit + structured |
...
| Ablasi-1: PDF only | L1 | 5 | 0.230 | 0.157 | 0.371 | 0.062‡ | 0.001 |
| Ablasi-2: PDF+DB | L1+L2 | 5 | 0.231 | 0.127 | 0.408 | 0.055‡ | 0.002 |
...
**Gambar 2c** membandingkan tiga metrik komposit (KTE, MSRS, AQI) antar skenario. MSRS tertinggi pada Skenario C (PostgreSQL, 0.825); KTE tertinggi Skenario D/PDF+DB (0.484); Skenario E memiliki KTE = 0.414 meski pertanyaannya paling kompleks.
...
**Keterbatasan Cakupan Evaluasi.** Evaluasi mencakup 25 pertanyaan di lima skenario dalam satu domain (platform transaksi instrumen keuangan domain-spesifik, BOND_SYS) dalam corpus berbahasa Indonesia. Validasi empiris pada domain dan bahasa lain diperlukan untuk memperkuat generalisasi hasil.
...
3. **Multi-sumber terbukti secara eksperimen:** Evaluasi batch 25 pertanyaan (5 skenario × 5 pertanyaan) menunjukkan Precision@K = 1.000 dan MRR = 1.000 di seluruh skenario. Performa agregat: A (Chat Only, *Overall* = 0.282, MSRS = 0.725), B (PDF, *Overall* = 0.276, MSRS = 0.625), C (PostgreSQL, *Overall* = 0.228, MSRS = 0.825), D (PDF+DB, *Overall* = 0.318, MSRS = 0.763), E (Hybrid, *Overall* = 0.358, MSRS = 0.713). Skenario E membuktikan klaim *source-agnostic* pada level tertinggi: *Retrieval Relevance* = 0.582 tertinggi di antara semua skenario, mengkonfirmasi query dapat menjangkau chunk relevan dari ketiga layer corpus secara bersamaan.
